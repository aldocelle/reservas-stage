...
async function load(){loading.value=true;error.value='';try{days.value=await Promise.all(nextDays().map(async d=>{const r=await fetch(`${API}/availability?date=${d.date}`);if(!r.ok)throw Error('No se pudo consultar disponibilidad');const x=await r.json();const slots=x.slots||[];return {...d,slots,used:slots.reduce((n,s)=>n+s.reserved,0),total:slots.reduce((n,s)=>n+s.capacity,0)}}));selectedDay.value=Math.max(0,days.value.findIndex(d=>d.slots.some(s=>s.available>0)));selectedSlot.value=0}catch(e){error.value=e.message;days.value=nextDays().map(d=>({...d,slots:[{slot_id:'demo',start_time:'14:00',end_time:'16:00',capacity:14,reserved:6,available:8},{slot_id:'demo2',start_time:'16:00',end_time:'18:00',capacity:14,reserved:9,available:5},{slot_id:'demo3',start_time:'18:00',end_time:'20:00',capacity:12,reserved:7,available:5}],used:22,total:40}));selectedDay.value=0;selectedSlot.value=0}finally{loading.value=false}}
